const createForm = (room_name, start_at, end_at, referrer, eventTitle) => {
...
const verifyStart = (start, end, resource, referrer, eventTitle) => {
  const form = document.createElement('form');
  form.classList.add('swal-form');
  if (resource) {
    form.innerHTML = createForm(resource.title, start, end, referrer, '');
  } else {
    form.innerHTML = createForm('', '', '', referrer, eventTitle);
  }
  const obj = {
    content: form,
    buttons: {
        ...
        .then((res) => {
          if (res.err) {
            ...
            swal({
              ...
              if (result) {
                if (resource) {
                  ...
                  verifyStart('', '', '', referrer, eventTitle);
                }
              }
            });
            ...
            if (resource) {
              ...
              verifyCode(res, '', '', '', referrer, eventTitle);
            }
          } else {
        ...
        swal({
          ...
          .then((res) => {
            if (resource) {
              ...
              verifyCode(response, '', '', '', referrer, eventTitle);
            }
          });
      } else if (value) {
